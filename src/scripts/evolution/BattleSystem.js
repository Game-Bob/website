export class BattleSystem {
    ...
    start(fighter1, fighter2, onBattleEnd) {
        ...
        this.ui.actionText.innerText = this.t("startBattle", "¡COMIENZA LA PELEA!");
        ...
        const battleInterval = setInterval(() => {
            ...
            setTimeout(() => {
                ...
                let msg = `${attacker.data.name}${this.t("unitHits", " golpea!")} -${Math.floor(dmg)}`;
                let color = isF1Turn ? "var(--color-blue-alt-light)" : "var(--color-red-alt-light)";

                if (isCrit) {
                    msg += ` ${this.t("critical", " ¡CRÍTICO!")}`;
                    color = "var(--color-warning)";
                } else if (typeMult > 1) {
                    msg += ` ${this.t("veryEffective", " ¡Es muy eficaz!")}`;
                } else if (typeMult < 1) {
                    msg += ` ${this.t("notVeryEffective", " No es muy eficaz...")}`;
            ...
            if (hp1 <= 0 || hp2 <= 0) {
                ...
                setTimeout(() => {
                    this.ui.actionText.innerText = `${winner.data.name}${this.t("wins", " GANA!")}`;
                    this.ui.actionText.style.color = "var(--color-warning)";
