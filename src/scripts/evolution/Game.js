...
export class Game {
    constructor(config) {
        this.base_elements = JSON.parse(JSON.stringify(config.base_elements));
        this.recipes = JSON.parse(JSON.stringify(config.recipes));
        ...
        this.base_elements.forEach(element => {
            if (this.i18n.entities && this.i18n.entities[element.id]) {
                element.name = this.i18n.entities[element.id];
                if (this.i18n.entities[element.id + "_desc"]) {
                    element.desc = this.i18n.entities[element.id + "_desc"];
                }
            }
        });

        this.recipes.forEach(recipe => {
            if (this.i18n.entities && this.i18n.entities[recipe.result.id]) {
                recipe.result.name = this.i18n.entities[recipe.result.id];
                if (this.i18n.entities[recipe.result.id + "_desc"]) {
                    recipe.result.desc = this.i18n.entities[recipe.result.id + "_desc"];
                }
            }
        });
        ...
        if (saved) {
            try {
                ...
                saveData.entities.forEach((e) => {
                    if (this.i18n.entities && this.i18n.entities[e.data.id]) {
                        e.data.name = this.i18n.entities[e.data.id];
                        if (this.i18n.entities[e.data.id + "_desc"]) {
                            e.data.desc = this.i18n.entities[e.data.id + "_desc"];
                        }
                    }
    ...
    spawnEgg() {
        this.spawnEntity({
            ...
            data: {
                id: "egg",
                name: this.i18n.entities?.egg?.name || "Huevo",
                icon: "mdi:egg",
                desc: "...",
    ...
    feedEntity(entityId, ingredient) {
        ...
        if (entity.data.id === "egg" && entity.stomach.length === 1) {
            entity.data.name = `${this.i18n.entities?.egg?.of || "Huevo de "}${ingredient.name}`;
    ...
    checkEvolution(entity) {
        ...
        if (recipe) {
            ...
            entity.data = {
                id: "poop",
                name: this.i18n.entities?.poop?.name || "Caca",
                icon: "mdi:emoticon-poop",
                desc: this.i18n.entities?.poop?.desc || "Huele mal.",
    ...
    renderEncyclopedia() {
            ...
            if (isDiscovered) {
                ...
                el.innerHTML = `
                    ...
                    <div class="w-full pt-3 border-t border-white/5">
                        <p class="text-[10px] uppercase tracking-widest text-slate-500 mb-2">${this.i18n.recipeLabel || "Receta"}</p>
                ...
                el.innerHTML = `
                    ...
                    <p class="text-xs text-slate-800 mb-4">${this.i18n.lockedLabel || "Bloqueado"}</p>
                `;
            }
        ...
        this.encyclopediaStats.textContent = `${this.t("discoveredCount", "Descubiertos")}: ${discoveredCount}/${this.recipes.length}`;
    }
}
