...
export function buildWidgetHtml(id: string, url: string, theme: string): string {
    ...
    const iframe = `\x3Ciframe id="${id}" src="${finalUrl}&id=${id}" width="100%" height="400" frameborder="0" scrolling="no" allow="camera" style="display:block;opacity:0;transition:opacity 0.4s,height 0.3s ease;border:none;" onload="window._jjshow()">\x3C/iframe>`;
...
export function buildPreviewHtml(id: string, url: string, theme: string): string {
    ...
    const { finalUrl, finalCleanUrl } = getFinalUrls(url, theme);
    const iframe = `<div style="width:100%;border:1px solid ${border};border-radius:12px;overflow:hidden;background:${bg};"><iframe id="${id}" src="${finalUrl}&id=${id}" width="100%" height="400" frameborder="0" scrolling="no" allow="camera" style="display:block;border:none;" onload="this.style.opacity='1'" loading="lazy"></iframe></div>`;
